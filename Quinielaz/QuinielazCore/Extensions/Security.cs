...
namespace BusinessLogic.Extensions
{
    public class Security
    {
        ...
        {
            bool Valid = false;
            quinielazdbEntities db = new quinielazdbEntities();
            try
            {
                ...
                if (Elements.Length > 0)
                {
                    ...
                    USER = db.tbl_usuarios.First(i => i.vchNombre == username && i.vchPassword == password);
                    if (USER.intUsuarioID == Convert.ToInt32(Elements[0]))
                        Valid = true;
                }
                db.Dispose();
                db = null;
